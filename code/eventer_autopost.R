...
# create_body() will create the content under the YAML header
 create_body <- function(title, description, authors, post_author){
...
# Write index.md -----
  ...
  ID <- "1R3udmA9BGETNPy1eJSNtogtlIqoNgeFXk7xILDL0cIM"
  ...
  post_df <- target %>% 
    ...
    rowwise() %>%
    mutate(yaml = create_yaml(`Model Name`, "admin", tags),
           body = create_body(
             title = `Model Name`,
